...
percentagecalculator: (inputs: Record<string, any>) => {
  ...
  const whole = Number(inputs.whole);
  const calculationType = String(inputs.calculationType || "basic");
  ...
  if (calculationType === "basic") {
    ...
    explanation = "Part divided by whole, multiplied by 100";
  }

  if (calculationType === "change") {
    result = ((whole - part) / part) * 100;
    explanation = "Difference between values divided by original value, multiplied by 100";
  }

  if (calculationType === "difference") {
    result = (Math.abs(part - whole) / ((part + whole) / 2)) * 100;
    explanation = "Absolute difference divided by average of both values, multiplied by 100";
  }
